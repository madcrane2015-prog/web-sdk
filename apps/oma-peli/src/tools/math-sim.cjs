...
//lisätty
  ...
  // RNG seed handling
  // rng_seed can be:
  //  - number (e.g. 1234) => deterministic
  //  - "random" / null / undefined => new random seed each run
  // ==========================================================
  const seedRaw = cfg.simulation?.rng_seed;

  const wantsRandom =
    seedRaw === null ||
    seedRaw === undefined ||
    String(seedRaw).toLowerCase() === "random";

  // Use full 32-bit unsigned seed for mulberry32
  const seed = wantsRandom
    ? crypto.randomInt(0, 0x100000000) // 0..2^32-1
    : (Number.isFinite(Number(seedRaw)) ? Number(seedRaw) : 0);

  const rng = mulberry32(seed >>> 0);

  console.log("RNG:", wantsRandom ? "auto-random seed" : "fixed seed", "| seed =", seed);
