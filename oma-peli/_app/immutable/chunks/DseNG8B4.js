...
`;let Le=null,jt=class{constructor(){Le||(Le=URL.createObjectURL(new Blob([xn],{type:"application/javascript"}))),this.worker=new Worker(Le)}};jt.revokeObjectURL=function(){Le&&(URL.revokeObjectURL(Le),Le=null)};const _n=`(function () {
    'use strict';
...
" class="svelte-1t2q5b9"><!></div> <button class="svelte-1t2q5b9"> </button> <button title="SPIN" class="svelte-1t2q5b9"></button>`,1);function Yn(t,e){$s(e,!0);const s=13,a=1445,r=1e3,n=1.75,i=-30,o=-10,d=160,f=0,l=720,h=750,g=.8,_=50,T=30,z=-40,F=1,L="height",G=Math.round(100*n),V=Math.round(G*(700/760)),Ye=V+10,fe=["a","b","c","d","e","f","g","h","i","j","k","l","emptyslot"],$=typeof window<"u"&&window.location.hostname.includes("github.io")?"/web-sdk/oma-peli/symbols":`${ze}/symbols`,Be={a:`${$}/Blue_hotrod.jpg`,b:`${$}/Blue_jacket.jpg`,c:`${$}/Blue_rollers.jpg`,d:`${$}/Blue_speakers.jpg`,e:`${$}/Premium_blonde.jpg`,f:`${$}/Premium_brunette.jpg`,g:`${$}/Premium_rocker.jpg`,h:`${$}/Red_bubblegum.jpg`,i:`${$}/Red_burger.jpg`,j:`${$}/Red_fries.jpg`,k:`${$}/Red_milkshake.jpg`,l:`${$}/Scatter.jpg`,emptyslot:`${$}/Emptyslot.jpg`},nt=`${$}/bg_base.jpg`,Gt=`${$}/ReelFrames.png`,Ht=`${$}/RockABillyReels_logo.png`,Vt={spin:`${ze}/sounds/spin.mp3`,stop:`${ze}/sounds/stop.mp3`,win:`${ze}/sounds/win.mp3`};let pe=U(!0),Ke={},Qe=null,xe=null,Ce=null,Fe=null,$e=U("Initializing..."),_e=U(""),Q=[],ie=U(1e3),H=U(10);const Yt=1,it=100;let he=U(!1),Ae=U(0),Me=U(!1),aa=U(0),ge=!1,se=null,ot=!1,lt=U(0),we=U(0),qe=U(0),ct=pr(()=>p(we)>0?(p(qe)/p(we)*100).toFixed(2):"0.00");const Xt={k:.18,j:.15,i:.15,c:.09,d:.09,b:.07,a:.07,f:.04,e:.03,g:.025,l:.025,h:.08,emptyslot:0};function Kt(u){if(u===6){const x=Math.random()<.5?"emptyslot":"h";return console.log(`Reel ${u} (MIDDLE): ${x}`),x}const m=Math.random();let c=0;const y=fe.filter(x=>x!=="emptyslot"&&x!=="h"),b=y.reduce((x,O)=>x+Xt[O],0);for(const x of y)if(c+=Xt[x]/b,m<c)return console.log(`Reel ${u} (WEIGHTED): ${x}`),x;return"k"}function Qt(){const u=[];for(let m=0;m<s;m++)u.push(Kt(m));return u}function ra(u){return u<3?{col:0,row:u}:u<6?{col:1,row:u-3}:u===6?{col:2,row:0}:u<10?{col:3,row:u-7}:{col:4,row:u-10}}function We(u,m){return u===0?m:u===1?3+m:u===2?6:u===3?7+m:u===4?10+m:-1}const na={k:{3:.15,4:.5,5:2.4},j:{3:.25,4:1,5:5},i:{3:.25,4:1,5:5},c:{3:.75,4:2.5,5:10},d:{3:.75,4:2.5,5:10},b:{3:1,4:3.5,5:12.5},a:{3:1,4:3.5,5:12.5},f:{3:1.5,4:7.5,5:25},e:{3:2.5,4:10,5:37.5},g:{3:2.5,4:12.5,5:50},h:{},l:{3:2.5,4:12.5,5:50},emptyslot:{}};function ia(){var x;const u=[],m=[];for(let O=0;O<s;O++)M[O]==="l"&&m.push(O);if(m.length>=5){const O=8+(m.length-5);console.log(`🎰 SCATTER WIN! ${m.length} scatters = ${O} FREE SPINS!`),u.push({symbol:"l",count:m.length,payout:0,positions:m})}M[0],M[1],M[2],M[3],M[4],M[5],M[6],M[7],M[8],M[9],M[10],M[11],M[12];const c=[];for(let O=0;O<3;O++)for(let w=0;w<3;w++)for(let R=0;R<1;R++)for(let I=0;I<3;I++)for(let A=0;A<3;A++){const N=[We(0,O),We(1,w),We(2,R),We(3,I),We(4,A)];c.push(N)}console.log(`Generated ${c.length} possible paths (should be 81)`);const y=new Map;for(const O of c){const w=O.map(A=>M[A]);if(w[0]==="emptyslot"||w[0]==="l")continue;let R=null;for(let A=0;A<w.length;A++)if(w[A]!=="h"&&w[A]!=="emptyslot"&&w[A]!=="l"){R=w[A];break}if(!R)continue;let I=0;for(let A=0;A<w.length;A++){const N=w[A];if(N===R||N==="h")I++;else break}if(I>=3){const A=`${R}-${I}`,N=y.get(A);N?N.lineCount++:y.set(A,{symbol:R,length:I,lineCount:1,examplePath:O.slice(0,I)})}}const b=[];for(const[O,w]of y.entries()){const R=(x=na[w.symbol])==null?void 0:x[w.length];if(R!==void 0&&R>0){const I=R*p(H);console.log(`Win: ${w.length}x ${w.symbol} (found on ${w.lineCount} ways) = ${I}`),b.push({symbol:w.symbol,count:w.length,payout:I,positions:w.examplePath})}}return u.push(...b),u}let me=U(0),dt=U(le([])),oe=U(!1),Je=U(!1);const oa={a:"Hot Rod",b:"Jacket",c:"Roller Skates",d:"Microphone",e:"Blonde",f:"Brunette",g:"Rockabilly",h:"WILD",i:"Burger",j:"Fries",k:"Milkshake",l:"SCATTER",emptyslot:"Empty"};function ut(u){if(!p(pe)||!Ke[u])return;const m=Ke[u];m.currentTime=0,m.play().catch(c=>{console.warn("Äänen toisto epäonnistui:",u,c)})}function la(u){je.forEach(m=>B.stage.removeChild(m)),je=[],u.forEach(m=>{m.positions.forEach(c=>{const y=ke[c],b=new It().rect(0,0,G,V).fill({color:16776960,alpha:0}).stroke({color:16766720,width:4});b.x=y.container.x,b.y=y.container.y,B.stage.addChild(b),je.push(b);let x=0,O=1;const w=.05;B.ticker.add(()=>{x+=O*w,x>=.4&&(O=-1),x<=0&&(O=1),b.alpha=.5+x})})})}function ft(){je.forEach(u=>B.stage.removeChild(u)),je=[]}let M=Qt(),pt,B,ke=[],je=[];class ca{constructor(m,c){Z(this,"index");Z(this,"container");Z(this,"offset",0);Z(this,"speed",0);Z(this,"targetSpeed",30);Z(this,"state","idle");Z(this,"stopDelay",0);Z(this,"bounceOffset",0);Z(this,"bounceSpeed",0);Z(this,"bounceFrames",0);this.index=m,this.container=c}start(m){this.state="spinning",this.speed=0,this.targetSpeed=35,this.stopDelay=m}update(){this.state!=="idle"&&(this.state==="spinning"&&(this.speed<this.targetSpeed&&(this.speed+=2),this.stopDelay>0?this.stopDelay--:this.state="slowing"),this.state==="slowing"&&(this.speed*=.92,this.speed<2.5&&(this.state="bouncing",this.bounceSpeed=-8,this.bounceFrames=0,this.speed=0,this.offset=0,ut("stop"))),this.state==="bouncing"&&(this.bounceFrames++,this.bounceSpeed+=.8,this.bounceOffset+=this.bounceSpeed,this.bounceOffset>3&&(this.bounceOffset=3,this.bounceSpeed*=-.6),(this.bounceFrames>45||Math.abs(this.bounceSpeed)<.5&&Math.abs(this.bounceOffset)<1)&&(this.state="stopped",this.bounceOffset=0,this.bounceSpeed=0)),this.speed>0&&(this.offset+=this.speed,this.offset>=Ye&&(this.offset=0,M[this.index]=Kt(this.index))))}draw(){const m=this.container;m.removeChildren();const c=M[this.index];if(!c||!Qe||!Qe[c])return;const y=Qe[c];if(!y)return;const b=this.offset+this.bounceOffset,x=new Ze(y);x.width=G,x.height=V,x.x=0,x.y=b,m.addChild(x)}}Ms(async()=>{B=new Ks,await B.init({width:a,height:r,background:"#001a33"}),pt.appendChild(B.canvas);const u={};try{S($e,"Loading background and UI images..."),Q.push(`Loading background: ${nt}`),Q.push(`Loading reel frames: ${Gt}`),Q.push(`Loading logo: ${Ht}`),await Te.load([{alias:"background",src:nt},{alias:"reelframes",src:Gt},{alias:"logo",src:Ht}]),xe=be.from("background"),Ce=be.from("reelframes"),Fe=be.from("logo"),console.log("✅ Background texture created:",xe.width,"x",xe.height),console.log("✅ Reel frames texture created:",Ce.width,"x",Ce.height),console.log("✅ Logo texture created:",Fe.width,"x",Fe.height),Q.push("✅ All UI images loaded"),S($e,"Loading symbols...");const c=[];for(const y of fe)c.push({alias:y,src:Be[y]});await Te.load(c);for(const y of fe){const b=Be[y];Q.push(`Loading symbol ${y}: ${b}`);try{const x=be.from(y);u[y]=x,console.log(`✅ Symbol ${y} loaded:`,x.width,"x",x.height),Q.push(`✅ Symbol ${y} loaded`)}catch(x){const O=`❌ Failed to load symbol ${y} from ${b}: ${x}`;throw Q.push(O),console.error(O),new Error(O)}}Qe=u,S($e,"Assets loaded successfully!")}catch(c){S(_e,`Asset loading failed: ${c}`),Q.push(p(_e)),console.error(p(_e));return}console.log("Ladataan ääniefektit...");for(const[c,y]of Object.entries(Vt)){const b=new Audio;b.src=y,b.preload="auto",b.volume=.7,b.load(),b.addEventListener("error",()=>{console.log(`Äänitiedostoa ei löydy: ${y} (käytetään hiljaista placeholderia)`)}),Ke[c]=b}console.log("Ladataan ääniefektit...");for(const[c,y]of Object.entries(Vt)){const b=new Audio;b.src=y,b.preload="auto",b.volume=.7,b.load(),b.addEventListener("error",()=>{console.log(`Äänitiedostoa ei löydy: ${y} (käytetään hiljaista placeholderia)`)}),Ke[c]=b}if(console.log("Taustakuva ladattu, tekstuuri:",xe),xe){const c=new Ze(xe);B.renderer.width/B.renderer.height;const y=c.texture.width/c.texture.height;let b;b=B.renderer.height/c.texture.height,b*=F,c.scale.set(b),c.x=(B.renderer.width-c.width)/2,c.y=(B.renderer.height-c.height)/2+z,B.stage.addChild(c),console.log("Taustakuva lisätty:",L,"mode, size:",c.width.toFixed(0),"x",c.height.toFixed(0),"image aspect:",y.toFixed(2),"scale:",b.toFixed(2),"pos:",c.x.toFixed(0),c.y.toFixed(0))}else console.error("Taustakuva ei ole saatavilla!");[{x:70,y:120},{x:203,y:120},{x:345,y:120},{x:487,y:120},{x:620,y:120}].map(c=>({x:c.x+i,y:c.y+o})),ke=[];for(let c=0;c<s;c++){const y=ra(c),b=y.col,x=y.row,O=300+b*(G+20),w=250+x*(V+15),R=c===6?O+f:O,I=c===6?w+d:w,A=new Nt;A.x=R+i,A.y=I+o;const N=[16711680,65280,255,16776960,16711935,65535,16753920,8388736,32768,128,16744448,8388863,33023],J=new It().rect(0,0,G,V).fill({color:N[c],alpha:.3});J.x=A.x,J.y=A.y,B.stage.addChild(J);const ve=new Ns({fontFamily:"Arial",fontSize:20,fill:16777215,fontWeight:"bold",stroke:{color:0,width:2}}),Y=new Fn({text:c.toString(),style:ve});Y.x=A.x+5,Y.y=A.y+5,B.stage.addChild(Y);const K=new It().rect(0,0,G,V).fill(16777215);K.x=A.x,K.y=A.y,A.mask=K,B.stage.addChild(K),B.stage.addChild(A),ke.push(new ca(c,A))}if(Ce){const c=new Ze(Ce);c.scale.set(1),c.x=250,c.y=200,B.stage.addChild(c),console.log("Reel frames lisätty:",c.width.toFixed(0),"x",c.height.toFixed(0))}if(Fe){const c=new Ze(Fe);c.scale.set(g),c.x=(B.renderer.width-c.width)/2+_,c.y=T,B.stage.addChild(c),console.log("Logo lisätty päällimmäiseen layeriin:",c.width.toFixed(0),"x",c.height.toFixed(0))}B.ticker.add(da)});function da(){for(const u of ke)u.update(),u.draw();if(!p(oe)&&!ot&&ke.every(u=>u.state==="stopped")){ot=!0;const u=ia();console.log(`Checking wins, found ${u.length} wins`),u.length>0?(S(dt,le(u)),S(me,le(u.reduce((m,c)=>m+c.payout,0))),ua(p(me)),S(oe,!0),S(aa,le(Date.now())),console.log(`🎉 VOITTO! ${p(me)} pistettä! Uusi saldo: ${p(ie)}`),console.log(`isShowingWin set to: ${p(oe)}, totalWin: ${p(me)}`),u.forEach(m=>{const c=m.multiplier?` (${m.multiplier}x multiplier)`:"";console.log(`${m.count}x ${m.symbol} = ${m.payout} pistettä${c}`)}),la(u),ut("win"),p(he)&&!ge&&(ge=!0,se=window.setTimeout(()=>{p(oe)&&p(he)&&(console.log("Autoplay: Auto-closing win popup after 1.5s"),S(oe,!1),ft()),se=window.setTimeout(()=>{ge=!1,gt()},200)},1500))):(console.log("No wins found this spin"),p(he)&&!ge&&(ge=!0,se=window.setTimeout(()=>{ge=!1,gt()},1e3)))}}function Jt(){if(p(ie)<p(H)){alert(`Not enough credits! Balance: ${p(ie)}, Bet: ${p(H)}`),ht();return}se!==null&&(clearTimeout(se),se=null),S(ie,p(ie)-p(H)),ys(lt),S(we,p(we)+p(H)),S(dt,le([])),S(me,0),S(oe,!1),ot=!1,ft(),M=Qt(),ke.forEach((u,m)=>u.start(60+m*10)),ut("spin")}function ua(u){S(ie,p(ie)+u),S(qe,p(qe)+u)}function fa(){p(H)<it&&S(H,le(Math.min(p(H)+1,it)))}function pa(){p(H)>Yt&&S(H,le(Math.max(p(H)-1,Yt)))}function ha(){S(H,it)}function Ne(u){S(he,!0),S(Ae,le(u)),S(Me,!1),gt()}function ht(){S(he,!1),S(Ae,0),ge=!1,se!==null&&(clearTimeout(se),se=null)}function gt(){if(!p(he)||p(Ae)<=0){ht();return}console.log(`Autoplay: Starting spin ${p(Ae)} rounds left`),Jt(),ys(Ae,-1)}function ga(){confirm("Reset all statistics?")&&(S(lt,0),S(we,0),S(qe,0))}var Zt=Vn(),es=Ve(Zt);{var ma=u=>{var m=Wn(),c=v(k(m),2),y=v(k(c));E(c);var b=v(c,2);{var x=Y=>{var K=$n(),Ue=v(k(K));E(K),ae(()=>W(Ue,` ${p(_e)??""}`)),j(Y,K)};ee(b,Y=>{p(_e)&&Y(x)})}var O=v(b,2),w=v(k(O));w.nodeValue=` ${ze??""}`,E(O);var R=v(O,2),I=v(k(R));I.nodeValue=` ${nt??""}`,E(R);var A=v(R,2),N=v(k(A));E(A);var J=v(A,2);{var ve=Y=>{var K=qn(),Ue=k(K),Ia=k(Ue);E(Ue);var Ba=v(Ue,2);ws(Ba,17,()=>Q,As,(Ca,Fa)=>{var Tt=Mn(),$a=k(Tt,!0);E(Tt),ae(()=>W($a,p(Fa))),j(Ca,Tt)}),E(K),ae(()=>W(Ia,`Log (${Q.length??""})`)),j(Y,K)};ee(J,Y=>{Q.length>0&&Y(ve)})}E(m),ae(()=>{W(y,` ${p($e)??""}`),W(N,` ${Be.a??""}`)}),j(u,m)};ee(es,u=>{(p($e)!=="Assets loaded successfully!"||p(_e))&&u(ma)})}var ts=v(es,2);{var va=u=>{var m=Un(),c=v(k(m),2),y=k(c);E(c);var b=v(c,2);ws(b,17,()=>p(dt),As,(O,w)=>{var R=Nn(),I=k(R),A=v(I);{var N=J=>{var ve=jn(),Y=k(ve);E(ve),ae(()=>W(Y,`(${p(w).multiplier??""}x kerroin!)`)),j(J,ve)};ee(A,J=>{p(w).multiplier&&p(w).multiplier>1&&J(N)})}E(R),ae(()=>W(I,`${p(w).count??""} × ${oa[p(w).symbol]??""} = ${p(w).payout??""} pistettä `)),j(O,R)});var x=v(b,2);E(m),ae(()=>W(y,`${p(me)??""} pistettä`)),q("click",x,()=>{S(oe,!1),ft(),console.log("Win popup closed, ready for next spin")}),j(u,m)};ee(ts,u=>{p(me)>0&&p(oe)&&u(va)})}var ss=v(ts,2);{var ba=u=>{var m=Dn(),c=v(k(m),8);E(m),q("click",c,()=>{S(Je,!1)}),j(u,m)};ee(ss,u=>{p(Je)&&u(ba)})}var as=v(ss,2);gr(as,u=>pt=u,()=>pt);var mt=v(as,2),vt=k(mt),rs=v(k(vt),2),ya=k(rs,!0);E(rs),E(vt);var ns=v(vt,2),is=v(k(ns),2),xa=k(is,!0);E(is),E(ns),E(mt);var bt=v(mt,2),os=v(k(bt),2),ls=k(os),yt=v(ls,2),_a=k(yt,!0);E(yt);var cs=v(yt,2),Aa=v(cs,2);E(os),E(bt);var ds=v(bt,2),xt=v(ds,2),_t=v(k(xt),2),us=v(k(_t),2),wa=k(us,!0);E(us),E(_t);var At=v(_t,2),fs=v(k(At),2),ka=k(fs,!0);E(fs),E(At);var wt=v(At,2),ps=v(k(wt),2),Ea=k(ps,!0);E(ps),E(wt);var kt=v(wt,2),Et=v(k(kt),2),Sa=k(Et);E(Et),E(kt);var Ta=v(kt,2);E(xt);var St=v(xt,2),La=k(St);{var Oa=u=>{var m=zn(),c=v(k(m),2),y=k(c);E(c);var b=v(c,2);E(m),ae(()=>W(y,`${p(Ae)??""} left`)),q("click",b,ht),j(u,m)},Pa=u=>{var m=Hn(),c=Ve(m),y=v(c,2);{var b=x=>{var O=Gn(),w=v(k(O),2),R=v(w,2),I=v(R,2),A=v(I,2),N=v(A,2),J=v(N,2);E(O),q("click",w,()=>Ne(10)),q("click",R,()=>Ne(100)),q("click",I,()=>Ne(1e3)),q("click",A,()=>Ne(1e4)),q("click",N,()=>Ne(1e5)),q("click",J,()=>{S(Me,!1)}),j(x,O)};ee(y,x=>{p(Me)&&x(b)})}q("click",c,()=>{S(Me,!p(Me))}),j(u,m)};ee(La,u=>{p(he)?u(Oa):u(Pa,!1)})}E(St);var Ee=v(St,2),Ra=k(Ee,!0);E(Ee);var hs=v(Ee,2);Se(hs,"style",`
  ...
  `),ae((u,m,c,y,b)=>{W(ya,u),W(xa,p(H)),W(_a,p(H)),W(wa,m),W(ka,c),W(Ea,y),Se(Et,"style",`color: ${b??""};`),W(Sa,`${p(ct)??""}%`),Se(Ee,"style",`
    position: absolute;
    top: 10px;
    right: 10px;
    width: 40px;
