...
`,_+=`${"=".repeat(60)}
`,r(ve).push(_)}function qr(){const c=`SLOT GAME WIN LOG
Generated: ${new Date().toLocaleString("fi-FI")}
${r(ve).join(`
`)}`,p=new Blob([c],{type:"text/plain"}),d=URL.createObjectURL(p),w=document.createElement("a");w.href=d,w.download=`win-log-${Date.now()}.txt`,w.click(),URL.revokeObjectURL(d)}function Jr(){y(ve,ye([]))}function en(){r(Z)||(y(Z,!0),y(Te,10),y(Ie,0),Le(zt),console.log("🎰 TEST MODE: Free spins activated! 10 spins granted."))}const tn={k:.08,j:.07,i:.07,c:.07,d:.07,b:.07,a:.07,f:.06,e:.05,g:.04,l:.1,h:0,emptyslot:.25},sn={k:0,j:0,i:0,c:.07,d:.07,b:.07,a:.07,f:.14,e:.12,g:.11,l:.1,h:0,emptyslot:.25};function ga(c){if(c===6)return Math.random()<.55?"h":"emptyslot";const p=r(Z)?sn:tn,d=Math.random();let w=0;const _=Oe.filter(L=>L!=="h"),k=_.reduce((L,D)=>L+p[D],0);for(const L of _)if(w+=p[L]/k,d<w)return L;return"f"}function ma(){const c=[];for(let p=0;p<a;p++){const d=ga(p);c.push(d),Le(St),d==="emptyslot"&&Le(ts)}return c}function ba(c){return c<3?{col:0,row:c}:c<6?{col:1,row:c-3}:c===6?{col:2,row:0}:c<10?{col:3,row:c-7}:{col:4,row:c-10}}function ht(c,p){return c===0?p:c===1?3+p:c===2?6:c===3?7+p:c===4?10+p:-1}function an(){if(r(Z)){const c=Math.random();return c<.7?3:c<.92?5:10}else{const c=Math.random();return c<.7?1:c<.92?2:3}}const rn={k:{3:.2,4:.6,5:1.5},j:{3:.4,4:1,5:2.5},i:{3:.4,4:1,5:2.5},c:{3:.8,4:2,5:5},d:{3:.8,4:2,5:5},b:{3:1.5,4:4,5:8},a:{3:1.5,4:4,5:8},f:{3:3,4:8,5:20},e:{3:5,4:10,5:25},g:{3:7,4:15,5:50},h:{},l:{},emptyslot:{}};function nn(){var fe;const c=[],p=[];for(let z=0;z<a;z++)vt[z]==="l"&&p.push(z);if(p.length>=5){const z=p.length;y(Te,r(Te)+z),r(Z)?console.log(`🎰 FREE SPINS RETRIGGERED! +${z} FREE SPINS! Total: ${r(Te)}`):(y(Z,!0),y(Ie,0),Le(zt),console.log(`🎰 FREE SPINS TRIGGERED! ${p.length} scatters = ${z} FREE SPINS!`)),c.push({symbol:"l",count:p.length,payout:0,positions:p,multiplier:1})}const d=[];for(let z=0;z<3;z++)for(let R=0;R<3;R++)for(let O=0;O<1;O++)for(let I=0;I<3;I++)for(let N=0;N<3;N++){const W=[ht(0,z),ht(1,R),ht(2,O),ht(3,I),ht(4,N)];d.push(W)}console.log(`Generated ${d.length} possible paths (should be 81)`);const w=[];for(const z of d){const R=z.map(W=>vt[W]);if(R[0]==="emptyslot"||R[0]==="l")continue;let O=null;for(let W=0;W<R.length;W++)if(R[W]!=="h"&&R[W]!=="emptyslot"&&R[W]!=="l"){O=R[W];break}if(!O)continue;let I=0;const N=new Set;for(let W=0;W<z.length;W++){const Be=z[W],{col:at}=ba(Be),ue=R[W];(ue===O||ue==="h")&&N.add(at)}for(let W=0;W<5&&N.has(W);W++)I++;if(I>=3){const Be=z[0]%3;w.push({symbol:O,length:I,path:z.slice(0,I),startRow:Be})}}const _=[],k=new Map;for(const z of w){const R=`${z.symbol}-${z.path.join(",")}`;k.has(R)||k.set(R,[]),k.get(R).push(z)}for(const[z,R]of k.entries()){const O=Math.max(...R.map(N=>N.length)),I=R.find(N=>N.length===O);I&&_.push(I)}const L=[],D=new Map;for(const z of _)D.has(z.symbol)||D.set(z.symbol,[]),D.get(z.symbol).push(z);const U=[];for(const[z,R]of D.entries()){const O=Math.max(...R.map(N=>N.length)),I=R.filter(N=>N.length===O);U.push(...I)}const q=new Map;for(const z of U){const R=`${z.symbol}-${z.length}`;q.has(R)||q.set(R,[]),q.get(R).push(z)}const M=U.length>0?an():1;for(const[z,R]of q.entries()){const O=R[0],I=(fe=rn[O.symbol])==null?void 0:fe[O.length];if(I!==void 0&&I>0){const N=new Map;for(const ue of R)for(let ne=0;ne<ue.length;ne++)N.has(ne)||N.set(ne,new Set),N.get(ne).add(ue.path[ne]);let W=1;for(let ue=0;ue<O.length;ue++){const ne=N.get(ue);W*=ne?ne.size:1}const Be=I*r(Q)*W*M;console.log(`  ${O.length}x${O.symbol}: ${W} ways × ${I}x × ${r(Q)} bet × ${M} mult = ${Be}`);const at=new Set;for(const ue of R)ue.path.forEach(ne=>at.add(ne));L.push({symbol:O.symbol,count:O.length,payout:Be,positions:Array.from(at),multiplier:M})}}return c.push(...L),c}let ge=$(0),qe=$(ye([])),Ce=$(!1),Je=$(!1);const xa={a:"Hot Rod",b:"Jacket",c:"Roller Skates",d:"Microphone",e:"Blonde",f:"Brunette",g:"Rockabilly",h:"WILD",i:"Burger",j:"Fries",k:"Milkshake",l:"SCATTER",emptyslot:"Empty"};function ss(c){if(!r(ze)||!Dt[c])return;const p=Dt[c];p.currentTime=0,p.play().catch(d=>{console.warn("Äänen toisto epäonnistui:",c,d)})}function on(c){gt.forEach(p=>H.stage.removeChild(p)),gt=[],c.forEach(p=>{p.positions.forEach(d=>{const w=et[d],_=new Ks().rect(0,0,de,te).fill({color:16776960,alpha:0}).stroke({color:16766720,width:4});_.x=w.container.x,_.y=w.container.y,H.stage.addChild(_),gt.push(_);let k=0,L=1;const D=.05;H.ticker.add(()=>{k+=L*D,k>=.4&&(L=-1),k<=0&&(L=1),_.alpha=.5+k})})})}function as(){gt.forEach(c=>H.stage.removeChild(c)),gt=[]}let vt=ma(),rs,H,et=[],gt=[];class ln{constructor(p,d){xe(this,"index");xe(this,"container");xe(this,"offset",0);xe(this,"speed",0);xe(this,"targetSpeed",30);xe(this,"state","idle");xe(this,"stopDelay",0);xe(this,"bounceOffset",0);xe(this,"bounceSpeed",0);xe(this,"bounceFrames",0);this.index=p,this.container=d}start(p){this.state="spinning",this.speed=0,r(Y)==="slow"?this.targetSpeed=20:r(Y)==="medium"?this.targetSpeed=35:this.targetSpeed=50,this.stopDelay=p}startSynchronized(p){const d=60+p*Nr;this.start(d)}update(){if(this.state!=="idle"){if(this.state==="spinning"&&(this.speed<this.targetSpeed&&(this.speed+=2),this.stopDelay>0?this.stopDelay--:this.state="slowing"),this.state==="slowing"){const p=r(Y)==="slow"?.88:r(Y)==="medium"?.92:.95;this.speed*=p,this.speed<2.5&&(this.state="bouncing",this.bounceSpeed=-8,this.bounceFrames=0,this.speed=0,this.offset=0,ss("stop"),Yr())}this.state==="bouncing"&&(this.bounceFrames++,this.bounceSpeed+=.8,this.bounceOffset+=this.bounceSpeed,this.bounceOffset>3&&(this.bounceOffset=3,this.bounceSpeed*=-.6),(this.bounceFrames>45||Math.abs(this.bounceSpeed)<.5&&Math.abs(this.bounceOffset)<1)&&(this.state="stopped",this.bounceOffset=0,this.bounceSpeed=0)),this.speed>0&&(this.offset+=this.speed,this.offset>=me&&(this.offset=0,vt[this.index]=ga(this.index)))}}draw(){const p=this.container;p.removeChildren();const d=vt[this.index];if(!d||!At||!At[d])return;const w=At[d];if(!w)return;const _=this.offset+this.bounceOffset,k=new Pt(w);k.width=de,k.height=te,k.x=0,k.y=_,p.addChild(k)}}xr(async()=>{H=new $r,await H.init({width:n,height:o,background:"#001a33"}),rs.appendChild(H.canvas);const c={};try{y(ut,"Loading background and UI images..."),be.push(`Loading background: ${jt}`),be.push(`Loading reel frames: ${ua}`),be.push(`Loading logo: ${pa}`),await rt.load([{alias:"background",src:jt},{alias:"reelframes",src:ua},{alias:"logo",src:pa}]),Ye=Ue.from("background"),dt=Ue.from("reelframes"),ft=Ue.from("logo"),console.log("✅ Background texture created:",Ye.width,"x",Ye.height),console.log("✅ Reel frames texture created:",dt.width,"x",dt.height),console.log("✅ Logo texture created:",ft.width,"x",ft.height),be.push("✅ All UI images loaded"),y(ut,"Loading symbols...");const d=[];for(const w of Oe)d.push({alias:w,src:Nt[w]});await rt.load(d);for(const w of Oe){const _=Nt[w];be.push(`Loading symbol ${w}: ${_}`);try{const k=Ue.from(w);c[w]=k,console.log(`✅ Symbol ${w} loaded:`,k.width,"x",k.height),be.push(`✅ Symbol ${w} loaded`)}catch(k){const L=`❌ Failed to load symbol ${w} from ${_}: ${k}`;throw be.push(L),console.error(L),new Error(L)}}At=c,y(ut,"Assets loaded successfully!")}catch(d){y(Xe,`Asset loading failed: ${d}`),be.push(r(Xe)),console.error(r(Xe));return}console.log("Ladataan ääniefektit...");for(const[d,w]of Object.entries(Mr)){const _=new Audio;_.src=w,_.preload="auto",_.volume=.7,_.load(),_.addEventListener("error",()=>{console.log(`Äänitiedostoa ei löydy: ${w} (käytetään hiljaista placeholderia)`)}),Dt[d]=_}if(console.log("Taustakuva ladattu, tekstuuri:",Ye),Ye){const d=new Pt(Ye);H.renderer.width/H.renderer.height;const w=d.texture.width/d.texture.height;let _;_=H.renderer.height/d.texture.height,_*=Ae,d.scale.set(_),d.x=(H.renderer.width-d.width)/2,d.y=(H.renderer.height-d.height)/2+le,H.stage.addChild(d),console.log("Taustakuva lisätty:",ce,"mode, size:",d.width.toFixed(0),"x",d.height.toFixed(0),"image aspect:",w.toFixed(2),"scale:",_.toFixed(2),"pos:",d.x.toFixed(0),d.y.toFixed(0))}else console.error("Taustakuva ei ole saatavilla!");[{x:70,y:120},{x:203,y:120},{x:345,y:120},{x:487,y:120},{x:620,y:120}].map(d=>({x:d.x+l,y:d.y+h})),et=[];for(let d=0;d<a;d++){const w=ba(d),_=w.col,k=w.row,L=300+_*(de+20),D=250+k*(te+15),U=d===6?L+u:L,q=d===6?D+v:D,M=new la;M.x=U+l,M.y=q+h;const fe=[16711680,65280,255,16776960,16711935,65535,16753920,8388736,32768,128,16744448,8388863,33023],z=new Ks().rect(0,0,de,te).fill({color:fe[d],alpha:.3});z.x=M.x,z.y=M.y,H.stage.addChild(z);const R=new wr({fontFamily:"Arial",fontSize:20,fill:16777215,fontWeight:"bold",stroke:{color:0,width:2}}),O=new Ui({text:d.toString(),style:R});O.x=M.x+5,O.y=M.y+5,H.stage.addChild(O);const I=new Ks().rect(0,0,de,te).fill(16777215);I.x=M.x,I.y=M.y,M.mask=I,H.stage.addChild(I),H.stage.addChild(M),et.push(new ln(d,M))}if(dt){const d=new Pt(dt);d.scale.set(1),d.x=250,d.y=200,H.stage.addChild(d),Vt=d,y(Ht,ye(d.width)),console.log("Reel frames lisätty:",d.width.toFixed(0),"x",d.height.toFixed(0)),console.log("Control panel leveys päivitetty:",r(Ht).toFixed(0))}if(ft){const d=new Pt(ft);d.scale.set(P),d.x=(H.renderer.width-d.width)/2+V,d.y=ae,H.stage.addChild(d),console.log("Logo lisätty päällimmäiseen layeriin:",d.width.toFixed(0),"x",d.height.toFixed(0))}ha(),H.ticker.add(cn)});function cn(){for(const c of et)c.update(),c.draw();if(!r(Ce)&&!Qt&&et.every(c=>c.state==="stopped")){Qt=!0,Gr();const c=nn();console.log(`Checking wins, found ${c.length} wins`),c.length>0?(y(qe,ye(c)),y(ge,ye(c.reduce((p,d)=>p+d.payout,0))),Zr(r(Qe),c,r(ge)),dn(r(ge)),y(Ce,!0),y(Kr,ye(Date.now())),console.log(`🎉 VOITTO! ${r(ge)} pistettä! Uusi saldo: ${r(Se)}`),console.log(`isShowingWin set to: ${r(Ce)}, totalWin: ${r(ge)}`),c.forEach(p=>{const d=p.multiplier?` (${p.multiplier}x multiplier)`:"";console.log(`${p.count}x ${p.symbol} = ${p.payout} pistettä${d}`)}),on(c),ss("win"),r(he)&&!je&&(je=!0,Ee=window.setTimeout(()=>{r(Ce)&&r(he)&&(console.log("Autoplay: Auto-closing win popup after 1.5s"),y(Ce,!1),as()),Ee=window.setTimeout(()=>{je=!1,os()},200)},1500))):(console.log("No wins found this spin"),r(he)&&!je&&(je=!0,Ee=window.setTimeout(()=>{je=!1,os()},1e3))),r(Z)&&r(Te)===0&&(console.log(`🎰 FREE SPINS ENDED! Total won: ${r(Ie)}`),setTimeout(()=>{alert(`Free Spins Ended!
Total Won: ${r(Ie).toFixed(2)}`),y(Z,!1),y(Ie,0)},2e3))}}function ya(){if(ee&&r(ke)&&!ee.playing()&&(ee.play(),console.log("🎵 Background music started on first spin")),Hr(),r(Z)&&r(Te)>0)Le(Te,-1),Le(es),console.log(`🎰 FREE SPIN! Remaining: ${r(Te)}`);else if(!r(Z)){if(r(Se)<r(Q)){alert(`Not enough credits! Balance: ${r(Se)}, Bet: ${r(Q)}`),ns();return}y(Se,r(Se)-r(Q)),Le(Qe),y(Ze,r(Ze)+r(Q))}Ee!==null&&(clearTimeout(Ee),Ee=null),y(qe,ye([])),y(ge,0),y(Ce,!1),Qt=!1,as(),vt=ma(),et.forEach((c,p)=>{c.startSynchronized(p)}),ss("spin")}function dn(c){if(y(Se,r(Se)+c),y(pt,r(pt)+c),r(Z)&&y(Ie,r(Ie)+c),c>0){Le(Zt);const p=c/r(Q);p>=10&&Xr(),Ve&&(p>=50?Ve.show():p>=20?setTimeout(()=>Ve.show(),200):p>=10&&setTimeout(()=>Ve.show(),400))}}function _a(){r(Q)<Kt&&y(Q,ye(Math.min(r(Q)+1,Kt)))}function wa(){r(Q)>va&&y(Q,ye(Math.max(r(Q)-1,va)))}function fn(){y(Q,Kt)}function mt(c){y(he,!0),y(Ke,ye(c)),y(Ne,!1),os()}function ns(){y(he,!1),y(Ke,0),je=!1,Ee!==null&&(clearTimeout(Ee),Ee=null)}function os(){if(!r(he)||r(Ke)<=0){ns();return}console.log(`Autoplay: Starting spin ${r(Ke)} rounds left`),ya(),Le(Ke,-1)}function un(){confirm("Reset all statistics?")&&(y(Qe,0),y(Ze,0),y(pt,0),y(Zt,0),y(zt,0),y(es,0),y(St,0),y(ts,0))}var ka=fl();Ao(c=>{var p=Zi();f(Pe(p)),j(c,p)});var Aa=Pe(ka);{var pn=c=>{var p=tl(),d=f(b(p),2),w=f(b(d));x(d);var _=f(d,2);{var k=O=>{var I=qi(),N=f(b(I));x(I),oe(()=>F(N,` ${r(Xe)??""}`)),j(O,I)};ie(_,O=>{r(Xe)&&O(k)})}var L=f(_,2),D=f(b(L));D.nodeValue=` ${Re??""}`,x(L);var U=f(L,2),q=f(b(U));q.nodeValue=` ${jt??""}`,x(U);var M=f(U,2),fe=f(b(M));x(M);var z=f(M,2);{var R=O=>{var I=el(),N=b(I),W=b(N);x(N);var Be=f(N,2);Mt(Be,17,()=>be,Ft,(at,ue)=>{var ne=Ji(),Hn=b(ne,!0);x(ne),oe(()=>F(Hn,r(ue))),j(at,ne)}),x(I),oe(()=>F(W,`Log (${be.length??""})`)),j(O,I)};ie(z,O=>{be.length>0&&O(R)})}x(p),oe(()=>{F(w,` ${r(ut)??""}`),F(fe,` ${Nt.a??""}`)}),j(c,p)};ie(Aa,c=>{(r(ut)!=="Assets loaded successfully!"||r(Xe))&&c(pn)})}var za=f(Aa,2);{var hn=c=>{var p=rl(),d=f(b(p),2),w=b(d);x(d);var _=f(d,2);Mt(_,17,()=>r(qe),Ft,(U,q)=>{var M=sl(),fe=b(M);x(M),oe(z=>F(fe,`${r(q).count??""} × ${xa[r(q).symbol]??""} = ${z??""} pistettä`),[()=>r(q).payout.toFixed(2)]),j(U,M)});var k=f(_,2);{var L=U=>{var q=al(),M=b(q),fe=b(M);x(M),x(q),oe(()=>F(fe,`✨ ${r(qe)[0].multiplier??""}x WIN MULTIPLIER! ✨`)),j(U,q)};ie(k,U=>{r(qe).length>0&&r(qe)[0].multiplier>1&&U(L)})}var D=f(k,2);x(p),oe(()=>F(w,`${r(ge)??""} pistettä`)),C("click",D,()=>{y(Ce,!1),as(),console.log("Win popup closed, ready for next spin")}),j(c,p)};ie(za,c=>{r(ge)>0&&r(Ce)&&c(hn)})}var Sa=f(za,2);{var vn=c=>{var p=nl(),d=f(b(p),8);x(p),C("click",d,()=>{y(Je,!1)}),j(c,p)};ie(Sa,c=>{r(Je)&&c(vn)})}var Ea=f(Sa,2);sr(Ea,c=>rs=c,()=>rs);var Et=f(Ea,2),Ta=b(Et);{var gn=c=>{var p=ol(),d=Pe(p),w=f(b(d),2),_=b(w,!0);x(w),x(d);var k=f(d,2),L=f(b(k),2),D=b(L,!0);x(L),x(k),oe(U=>{F(_,r(Te)),F(D,U)},[()=>r(Ie).toFixed(2)]),j(c,p)};ie(Ta,c=>{r(Z)&&c(gn)})}var is=f(Ta,2),La=f(b(is),2),mn=b(La,!0);x(La),x(is);var Ra=f(is,2),$a=f(b(Ra),2),bn=b($a,!0);x($a),x(Ra),x(Et);var ls=f(Et,2),Pa=f(b(ls),2),Oa=b(Pa),cs=f(Oa,2),xn=b(cs,!0);x(cs);var Ia=f(cs,2),yn=f(Ia,2);x(Pa),x(ls);var Ca=f(ls,2),ds=f(Ca,2),Ba=f(b(ds),2);Ba.textContent=`v${s}`;var fs=f(Ba,2),Fa=f(b(fs),2),_n=b(Fa,!0);x(Fa),x(fs);var us=f(fs,2),Ma=f(b(us),2),wn=b(Ma,!0);x(Ma),x(us);var ps=f(us,2),Wa=f(b(ps),2),kn=b(Wa,!0);x(Wa),x(ps);var hs=f(ps,2),vs=f(b(hs),2),An=b(vs);x(vs),x(hs);var gs=f(hs,2),ms=f(b(gs),2),zn=b(ms);x(ms),x(gs);var bs=f(gs,2),Na=f(b(bs),2),Sn=b(Na,!0);x(Na),x(bs);var xs=f(bs,2),ja=f(b(xs),2),En=b(ja,!0);x(ja),x(xs);var ys=f(xs,2),Da=f(b(ys),2),Tn=b(Da);x(Da),x(ys);var Ua=f(ys,2),Ga=f(Ua,2),tt=b(Ga),Ln=b(tt);x(tt);var Tt=f(tt,2),bt=f(Tt,2),Rn=b(bt);x(bt);var _s=f(bt,2);x(Ga),x(ds);var ws=f(ds,2),$n=b(ws);{var Pn=c=>{var p=il(),d=f(b(p),2),w=b(d);x(d);var _=f(d,2);x(p),oe(()=>F(w,`${r(Ke)??""} left`)),C("click",_,ns),j(c,p)},On=c=>{var p=cl(),d=Pe(p),w=f(d,2);{var _=k=>{var L=ll(),D=f(b(L),2),U=f(D,2),q=f(U,2),M=f(q,2),fe=f(M,2),z=f(fe,2);x(L),C("click",D,()=>mt(10)),C("click",U,()=>mt(100)),C("click",q,()=>mt(1e3)),C("click",M,()=>mt(1e4)),C("click",fe,()=>mt(1e5)),C("click",z,()=>{y(Ne,!1)}),j(k,L)};ie(w,k=>{r(Ne)&&k(_)})}C("click",d,()=>{y(Ne,!r(Ne))}),j(c,p)};ie($n,c=>{r(he)?c(Pn):c(On,!1)})}x(ws);var st=f(ws,2),In=b(st,!0);x(st);var Lt=f(st,2),ks=b(Lt);A(ks,"src",`${K??""}/Control_leftend.png`),A(ks,"style",`height: ${g}px; flex-shrink: 0;`);var Rt=f(ks,2);A(Rt,"style",`
    flex-grow: 1;
    height: ${g}px;
  ...
  `);var As=b(Rt),Ha=f(b(As),2),zs=b(Ha);A(zs,"style",`
          ...
          cursor: pointer;
          background-color: transparent;
        `);var Ms=f($t,2),Mn=b(Ms,!0);x(Ms);var Wn=f(Ms,2);{var Nn=c=>{var p=dl(),d=f(b(p),2),w=f(d,2),_=f(w,2);x(p),oe(()=>{A(d,"style",`
            ...
            `),A(_,"style",`
              ...
              font-size: 12px;
              transition: all 0.2s;
            `)}),C("click",d,()=>{y(Y,"slow"),y(He,!1)}),C("click",w,()=>{y(Y,"medium"),y(He,!1)}),C("click",_,()=>{y(Y,"fast"),y(He,!1)}),j(c,p)};ie(Wn,c=>{r(He)&&c(Nn)})}x(Fs);var Ws=f(Fs,2);A(Ws,"src",`${K??""}/Control_divider.png`),A(Ws,"style",`height: ${g*.8}px; flex-shrink: 0;`);var Ns=f(Ws,2),js=b(Ns),jn=f(js,2);x(Ns);var Ds=f(Ns,2);A(Ds,"src",`${K??""}/Control_divider.png`),A(Ds,"style",`height: ${g*.8}px; flex-shrink: 0;`);var Us=f(Ds,2),Xa=f(b(Us),2),Dn=b(Xa,!0);x(Xa),x(Us);var Gs=f(Us,2);A(Gs,"src",`${K??""}/Control_divider.png`),A(Gs,"style",`height: ${g*.8}px; flex-shrink: 0;`);var Ka=f(Gs,2);A(Ka,"style",`
        width: 50px;
        height: 50px;
        ...
        cursor: pointer;
        background-color: transparent;
      `),x(Rt);var Qa=f(Rt,2);A(Qa,"src",`${K??""}/Control_rightend.png`),A(Qa,"style",`height: ${g}px; flex-shrink: 0;`),x(Lt);var Un=f(Lt,2);const Gn=De(()=>r(ge)/r(Q)>=50?"jackpot":r(ge)/r(Q)>=20?"medium":"small");sr(Qi(Un,{get winLevel(){return r(Gn)},get winAmount(){return r(ge)}}),c=>Ve=c,()=>Ve),oe((c,p,d,w,_,k,L,D,U)=>{A(Et,"style",`
  position: absolute;
  top: 20px;
  right: 20px;
  background: rgba(0, 0, 0, 0.85);
  ...
  z-index: 1500;
  min-width: 180px;
`),F(mn,c),F(bn,r(Q)),F(xn,r(Q)),F(_n,p),F(wn,d),F(kn,w),A(vs,"style",`color: ${_??""};`),F(An,`${r(qt)??""}%`),A(ms,"style",`color: ${k??""};`),F(zn,`${r(Jt)??""}%`),F(Sn,r(zt)),F(En,r(es)),F(Tn,`${r(Qr)??""}%`),tt.disabled=r(ve).length===0,A(tt,"style",`
        width: 100%;
        padding: 5px;
